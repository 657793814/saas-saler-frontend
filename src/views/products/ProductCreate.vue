          ...
          <div v-for="specId in selectedSpecs" :key="specId" class="spec-section">
            ...
            <el-form-item>
              <el-tag
                  ...
                  :key="value.id"
                  closable
                  @close="removeSpecValue(specId, value.id)"
                  class="spec-tag">
                {{ value.value }}
              </el-tag>
              <el-input
      ...
      await productFormRef.value.validate(async (valid) => {
        if (valid) {
          ...
          try {
            ...
            formData.append('summary', productForm.summary);
            formData.append('description', productForm.description);

            // 添加图片文件URL（已上传的图片）
            productForm.images.forEach((file, index) => {
              if (file.url) {
                formData.append(`imageUrls[${index}]`, file.url);
              }
            });
            ...
            skuList.value.forEach((sku, index) => {
              ...
              formData.append(`skus[${index}][costPrice]`, sku.costPrice);
              formData.append(`skus[${index}][purchasePrice]`, sku.purchasePrice);
              formData.append(`skus[${index}][stock]`, sku.stock);

              // 添加规格信息
              sku.specifications.forEach((spec, specIndex) => {
                formData.append(`skus[${index}][specifications][${specIndex}][specId]`, spec.specId);
                formData.append(`skus[${index}][specifications][${specIndex}][specName]`, spec.specName);
                formData.append(`skus[${index}][specifications][${specIndex}][valueId]`, spec.valueId);
                formData.append(`skus[${index}][specifications][${specIndex}][value]`, spec.value);
              });
            });
